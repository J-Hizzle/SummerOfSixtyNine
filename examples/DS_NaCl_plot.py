...
plt.sca(ax[0, 0])
plt.plot(n_cut_vals[1:10], energies_kJmol[1:10], 'bo')
...
plt.sca(ax[0, 1])
plt.plot(n_cut_vals[1:10], madelungs[1:10] , 'ro')
...
plt.sca(ax[1, 0])
plt.plot(n_cut_vals[:10], timings[:10], 'go')
plt.ylabel(r'$t_{run}$ in $\mathrm{s}$')
plt.title(r'Execution time')
plt.xlabel(r'$n_{cut}$')

plt.sca(ax[1, 1])
plt.plot(n_cut_vals[:10], mem_uses[:10], 'o', color='purple')
...
fig.savefig(fname='../data/direct_sum_results_nto10.png', dpi=900)
# %%